...
import { Search, ShoppingCart, PackageX, Pill, Check, Plus, Pencil, Boxes } from "lucide-react";
...
import type { Page, Product, Named } from "../lib/types";
...
export default function Catalogue() {
  ...
  const url = `/products?page_size=200&q=${encodeURIComponent(query)}${cat ? `&category_id=${cat}` : ""}`;
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={canManage ? "Browse and manage the products shops can order" : "Browse and order wholesale medical products"}
        action={canManage && (
            ...
            {data.items.map((p) => {
              ...
              return (
                <div key={p.id} className="card p-4 flex flex-col hover:shadow-pop hover:-translate-y-0.5 transition-all duration-200">
                  ...
                  {isCustomer && (
                    <button className={`mt-3 w-full py-2 ${justAdded ? "btn-success" : "btn-primary"}`}
                      disabled={p.available_stock <= 0} onClick={() => addToCart(p)}>
                  ...
                  {canManage && (
                    <div className="mt-3 flex gap-2">
                      <button className="btn-ghost flex-1 py-1.5" onClick={() => { setEditing(p); setFormOpen(true); }}>
                        <Pencil size={14} /> Edit
                      </button>
                      {canStock && (
                        <button className="btn-ghost flex-1 py-1.5" onClick={() => setBatchFor(p)}>
                          <Boxes size={14} /> Add stock
                        </button>
                      )}
                    </div>
                  )}
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 z-[60] rounded-xl text-white text-sm px-4 py-2.5 shadow-pop flex items-center gap-2 animate-scale-in ${toast.ok ? "bg-emerald-600" : "bg-rose-600"}`}>
